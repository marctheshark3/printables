...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--thin-fail-frac", type=float, default=0.20,
                    help="HARD only when a large fraction of edges are below thr (tessellation is noisy)")
    ...
    p.add_argument("--p1s-bed", type=float, default=256.0)
...
def main() -> int:
    ...
    # P1S bed (print XY = model XY by convention unless overridden)
    if dx >= args.p1s_bed or dy >= args.p1s_bed:
        hard_fail(f"G-bed: XY {dx:.1f}×{dy:.1f} exceeds P1S bed {args.p1s_bed}")
    ...
    # Thin edges: tessellation creates many short chords on curves — use a low thr
    # and only HARD when a large fraction is extremely short (knife-edge smell).
    if tris:
        ...
        if edge_lens:
            ...
            if frac > args.thin_fail_frac:
                hard_fail(
                    f"G-thin: {frac*100:.1f}% edges < {thin_thr:.2f} mm "
                    f"— likely knife edges / non-printable thin features"
                )
            elif frac > args.thin_fail_frac * 0.5:
                soft_warn(
                    f"G-thin: {frac*100:.1f}% edges under {thin_thr:.2f} mm "
                    f"(often tessellation; verify min wall ≥ {min_feature} mm)"
                )
