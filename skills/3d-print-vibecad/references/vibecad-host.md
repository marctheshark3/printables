...
2. Set `cad.backend: vibecad` and `cad.parametric: true` for dimensional work. Source files must be project-relative Python/VibeScript with `identifier =` for every PRINT_SPEC dimension. `.FCStd` or Markdown alone is not a source. Upstream FreeCAD is not this kernel.
...
6. OpenSCAD is not the dimensional default when VibeCAD resolves. If VibeCAD cannot run, use `3d-print-openscad`. Do not invent a kernel fallback inside VibeCAD.
